...
#TP    def get_CHAOS(self,datakey,as_output = False,ref_frame='ecef'):
#TP        
#TP        if all([i in self.data[datakey] for i in ['Bx_chaos','By_chaos','Bz_chaos']]):
#TP            print('Mag. field from CHAOS already calculated for '+datakey+'.')
#TP            return
#TP        if as_output:
#TP            return get_CHAOSmag(self.data[datakey],as_output = True,ref_frame=ref_frame)
#TP        get_CHAOSmag(self.data[datakey],as_output=False,ref_frame = ref_frame)
#TP    
#TP
#TP    def get_spacecraft_speed(self,datakey='EFD_ELF',ref_frame='ecef',regularize_speed = False,dt_lowfilt=20):
#TP        """
#TP        Compute spacecraft velocity from lat,lon,alt and time contained in the L2 
#TP        data using central finite differences
#TP        
#TP        Parameters
#TP        ----------
#TP        datakey : str
#TP            key of self.data denoting the dataset in which one wants to compute
#TP            the spacecraft speed.
#TP            
#TP        ref_frame : str
#TP            desired reference frame for the output the spacecraft speed.
#TP
#TP            'wgs84' : this SHOULD be the frame of the data given by the chineses
#TP            'ecef' or 'wgs84' : this is the wgs84 (cartesian) coordinate system 
#TP            'wgs84_spherical' or 'geo': geographic ref. frame 
#TP                WARNING! GEO FRAME NOT TESTED. COULD BE WRONG!
#TP
#TP        regularize_speed : bool
#TP            to be implemented (fifpy is missing in the module)
#TP        dt_lowfilt : float
#TP            to be implemented, inverse frequency cut of the low-pass fif filter
#TP        
#TP        Output
#TP        ------
#TP        no output is given. The result is stored in self.data[datakey] in the
#TP        ['vsx','vsy','vsz'] keys in the selected reference frame
#TP        """
#TP
#TP        if not hasattr(self.data,datakey):
#TP            raise ValueError('You must load the data for the desired input datakey :'+datakey)
#TP       
#TP        df = self.data[datakey]
#TP        
#TP        from .blombly.math.derivFD import derivfield as deriv #central finite differences derivative 
#TP        orbits = list(set(df.orbitn.values))
#TP
#TP        def get_speed_orbit(data):
#TP            t = data.index.values.astype(float)/1e9 #dt in seconds
#TP            t-=t[0]
#TP            if regularize_speed:
#TP                from scipy.interpolate import splrep,splev
#TP                nskip = CSES_PACKETSIZE[datakey]
#TP                MM = int(dt_lowfilt//np.diff(t[::nskip]).mean())
#TP                if MM == 1:
#TP                    print('WARNING: dt_lowfilt < temporal resolution! Skipping lowfiltering!')
#TP                    x,y,z = convert_GPS_to_ECEF(data.lat.values,data.lon.values,data.alt.values)
#TP                    vx = deriv(x,t); vy = deriv(y,t); vz = deriv(z,t)
#TP                else:
#TP                    tt = t[::nskip]
#TP                    x,y,z = convert_GPS_to_ECEF(data.lat.values[::nskip],data.lon.values[::nskip],data.alt.values[::nskip])
#TP                    vx,vy,vz = deriv(x,tt),deriv(y,tt),deriv(z,tt)
#TP                    vx,vy,vz = fif_lowfilter([vx,vy,vz],MM)
#TP                    #SPLINE INTERPOLATION TO FULL CADENCE
#TP                    tck = splrep(tt,vx); vx = splev(t,tck)
#TP                    tck = splrep(tt,vy); vy = splev(t,tck)
#TP                    tck = splrep(tt,vz); vz = splev(t,tck)
#TP            else:
#TP                x,y,z = convert_GPS_to_ECEF(data.lat.values,data.lon.values,data.alt.values)
#TP                vx = deriv(x,t); vy = deriv(y,t); vz = deriv(z,t)
#TP
#TP            if ref_frame == 'wgs84_spherical' or ref_frame == 'geo':
#TP                
#TP                x,y,z = convert_GPS_to_ECEF(data.lat.values,data.lon.values,data.alt.values)
#TP                #now converting to vlat, -vr, and vphi
#TP                cost = (z /np.sqrt(x**2+y**2+z**2))#[:-1]
#TP                sint = np.sqrt(1-cost**2)
#TP                phi =np.arctan2(y,x); cosp=np.cos(phi); sinp=np.sin(phi)#[:-1]
#TP                vr = vx*sint*cosp + vy*sint*sinp + vz*cost
#TP                vt = vx*cost*cosp + vy*cost*sinp - vz*sint
#TP                vp = -vx*sinp +vy*cosp
#TP                
#TP                vx = -vt #v_lat
#TP                vy =  vp #v_lon
#TP                vz = -vr #v_radial
...
#TP
#TP        if len(orbits) == 1:
#TP            get_speed_orbit(df)
#TP            return
#TP        for iorbit in orbits:
#TP            data = df[df.orbitn == iorbit]
#TP            get_speed_orbit(data)
#TP            df['vsx'] = data['vsx']
#TP            df['vsy'] = data['vsy']
#TP            df['vsz'] = data['vsz']
################################################################################
