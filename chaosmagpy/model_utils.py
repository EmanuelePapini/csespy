...
def synth_values(coeffs, radius, theta, phi, *, nmax=None, nmin=None,
    ...
    @njit
    def inner_loop_numba_true(nmin,nmax,B_radius,B_theta,B_phi,\
        Pnm,r_n,coeffs,radius,phi,source,where_poles,sinth,nt):
        num = 0
        for n in range(nmin, nmax+1):

            for it in range(nt):
                if source == 'internal':
                    B_radius[it] = B_radius[it] + (n+1) * Pnm[n, 0,it] * r_n[it] * coeffs[it, num]
                else:
                    B_radius[it] = B_radius[it] -n * Pnm[n, 0,it] * r_n[it] * coeffs[it, num]

                B_theta[it] += -Pnm[0, n+1,it] * r_n[it] * coeffs[it, num]
            ...
            for m in range(1, min(n, mmax)+1):
                ...
                for it in range(nt):
                    ...
                    if source == 'internal':
                        B_radius[it] = B_radius[it] + (n+1) * Pnm[n, m,it] * r_n[it]\
                                     * (coeffs[it, num] * cmp
                                        + coeffs[it, num+1] * smp)
                    else:
                        B_radius[it] = B_radius[it] -n * Pnm[n, m,it] * r_n[it]\
                                      * (coeffs[it, num] * cmp
                                         + coeffs[it, num+1] * smp)

                    B_theta[it] += (-Pnm[m, n+1,it] * r_n[it]
                                * (coeffs[it, num] * cmp
                                   + coeffs[it, num+1] * smp))
                    ...
                    if it in where_poles:
                        div_Pnm = Pnm[m, n+1,it]*Pnm[1, 0,it]
                    else:
                        div_Pnm = Pnm[n, m,it]/sinth[it]
                    
                    B_phi[it] += m * div_Pnm * r_n[it]\
                              * (coeffs[it, num] * smp
                                 - coeffs[it, num+1] * cmp)
        ...
        return B_radius, B_theta, B_phi
    @njit
    def inner_loop_numba_false(nmin,nmax,B_radius,B_theta,B_phi,\
        Pnm,r_n,coeffs,radius,phi,source,sinth,nt):
        num = 0
        for n in range(nmin, nmax+1):
    ...
    #print(r_n.shape)
    #print(coeffs.shape)
    #print(B_radius.shape)
    if poles:
        return inner_loop_numba_true(nmin,nmax,B_radius,B_theta,B_phi,\
            Pnm,r_n,coeffs,radius,phi,source,where_poles,sinth,grid_shape[0])
    else:
        return inner_loop_numba_false(nmin,nmax,B_radius,B_theta,B_phi,\
            Pnm,r_n,coeffs,radius,phi,source,sinth,grid_shape[0])
...
def legendre_poly(nmax, theta):
    ...
    @njit
    def inner_loop1(nmax,rootn,Pnm,costh,sinth):
        for m in range(nmax):
            ...
            for n in range(m+2, nmax+1):
                ...
                Pnm[n, m] = ((e * costh * Pnm[n-1, m] - rootn[d-e] * Pnm[n-2, m])
                             / rootn[d])
        return Pnm
    Pnm = inner_loop1(nmax,rootn,Pnm,costh,sinth)
    # dP(n,m) = Pnm(m,n+1) is the derivative of P(n,m) vrt. theta
    Pnm[0, 2] = -Pnm[1, 1]
    Pnm[1, 2] = Pnm[1, 0]
    @njit
    def inner_loop2(nmax,Pnm):
